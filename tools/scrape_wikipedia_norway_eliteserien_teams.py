...
SEASON_URL = "https://en.wikipedia.org/wiki/2025_Eliteserien"

def get_norway_eliteserien_teams():
...
def main():
    teams = get_norway_eliteserien_teams()
    teams_with_logos = []
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Norveç Ligi olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Norveç Ligi"] = teams_with_logos
        f.seek(0)
