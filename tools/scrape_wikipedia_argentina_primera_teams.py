...
SEASON_URL = "https://en.wikipedia.org/wiki/2025_Argentine_Primera_Division"

def get_argentina_primera_teams():
    resp = requests.get(SEASON_URL, headers=HEADERS)
    soup = BeautifulSoup(resp.text, "html.parser")
    teams = set()
    # "Teams" veya "Clubs" başlığı altındaki bağlantıları bul
    for header in soup.find_all(['h2', 'h3']):
        if 'team' in header.get_text().strip().lower() or 'club' in header.get_text().strip().lower():
    ...
    # Eğer yukarıda bulunamazsa, sayfadaki tüm wikitable'lardaki ilk sütun linklerini de ekle (yedek)
    if not teams:
...
def get_team_wikipedia_url(team_name):
    # Önce İngilizce Wikipedia'da doğrudan sayfa var mı bak
    ...
    if resp.status_code == 200 and 'may refer to:' not in resp.text:
        return direct_url
    # Sonra İspanyolca Wikipedia'da doğrudan sayfa var mı bak
    es_url = f"https://es.wikipedia.org/wiki/{team_name.replace(' ', '_')}"
    resp = requests.get(es_url, headers=HEADERS)
    if resp.status_code == 200 and 'puede referirse a:' not in resp.text:
        return es_url
    # Fallback: İngilizce Wikipedia'da arama yap
...
def main():
    ...
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Arjantin Premier Lig olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Arjantin Premier Lig"] = teams_with_logos
        f.seek(0)
