...
SEASON_URL = "https://en.wikipedia.org/wiki/2025%E2%80%9326_Belgian_Pro_League"

def get_belgian_league_teams():
...
def main():
    teams = get_belgian_league_teams()
    teams_with_logos = []
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Belgian Pro League olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Belgian Pro League"] = teams_with_logos
        f.seek(0)
