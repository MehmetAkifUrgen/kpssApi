...
SEASON_URL = "https://en.wikipedia.org/wiki/2025–26_Czech_First_League"

def get_czech_first_league_teams():
...
def main():
    teams = get_czech_first_league_teams()
    teams_with_logos = []
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Çek Cumhuriyeti Ligi olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Çek Cumhuriyeti Ligi"] = teams_with_logos
        f.seek(0)
