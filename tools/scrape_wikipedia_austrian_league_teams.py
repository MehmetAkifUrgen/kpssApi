...
SEASON_URL = "https://en.wikipedia.org/wiki/2025%E2%80%9326_Austrian_Football_Bundesliga"

def get_austrian_league_teams():
...
def main():
    teams = get_austrian_league_teams()
    teams_with_logos = []
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Austrian Bundesliga olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Austrian Bundesliga"] = teams_with_logos
        f.seek(0)
