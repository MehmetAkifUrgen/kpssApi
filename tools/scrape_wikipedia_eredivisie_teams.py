...
SEASON_URL = "https://en.wikipedia.org/wiki/2025%E2%80%9326_Eredivisie"

def get_eredivisie_teams():
...
def main():
    ...
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Eredivisie olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Eredivisie"] = teams_with_logos
        f.seek(0)
