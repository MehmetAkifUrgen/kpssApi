...
SEASON_URL = "https://en.wikipedia.org/wiki/2025_Campeonato_Brasileiro_S%C3%A9rie_A"

def get_brazil_serie_a_teams():
...
def main():
    teams = get_brazil_serie_a_teams()
    teams_with_logos = []
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Brezilya Série A olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Brezilya Série A"] = teams_with_logos
        f.seek(0)
