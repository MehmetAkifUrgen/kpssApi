...
SEASON_URL = "https://en.wikipedia.org/wiki/2025%E2%80%9326_Super_League_Greece"

def get_greece_super_league_teams():
    resp = requests.get(SEASON_URL, headers=HEADERS)
    soup = BeautifulSoup(resp.text, "html.parser")
    teams = set()
    # 1. "Teams" veya "Clubs" başlığı altındaki bağlantıları bul
...
def main():
    teams = get_greece_super_league_teams()
    teams_with_logos = []
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında Yunanistan Süper Ligi olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["Yunanistan Süper Ligi"] = teams_with_logos
        f.seek(0)
