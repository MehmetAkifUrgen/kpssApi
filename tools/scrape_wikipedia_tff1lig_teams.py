...
SEASON_URL = "https://en.wikipedia.org/wiki/2025%E2%80%9326_TFF_First_League"

def get_tff1lig_teams():
...
def main():
    ...
    for name in teams:
        ...
        time.sleep(0.5)
    # football.json'da teams altında TFF 1. Lig olarak ekle
    with open("../football.json", "r+", encoding="utf-8") as f:
        data = json.load(f)
        if "teams" not in data:
            data["teams"] = {}
        data["teams"]["TFF 1. Lig"] = teams_with_logos
        f.seek(0)
